...
extern int32_t last_interp_roll;
extern int32_t last_interp_pitch;

extern int32_t last_recorded_roll;
extern int32_t last_recorded_pitch;

extern float TIME_DIST;
...
void handlePenMotionAndSendSPI(SPIStruct* spiStruct){
  ...
  switch(current_state){
    ...
    case RECORDING:
      ...
      if(current_state == RECORDING){
        ...
        if(!send_coordinate){ // caso Overflow: QUESTA SOLUZIONE DEVO ANCORA VERIFICARLA
          return;
        }   
    ...
    case DRAW_RECORD:
      ...
      if (!abort_sequence && current_state == SEND_LAST_POINT) {
        //  Cambio immediato di stato!
        //setLed(&ledRed, true); // qui ci arriva? qui ci arriva? SI
        return;
      }
    ...
    case OVERFLOW_TRAJ:
      ...
      spiStruct->rollTX = last_recorded_roll;
      spiStruct->pitchTX = last_recorded_pitch;
    ...
    case SEND_LAST_POINT: 
      ...
      //prepareSPIData(spiStruct->rollTX, spiStruct->pitchTX, tx);
      spiStruct->rollTX = last_interp_roll; //  RIPARTO DALL'ULTIMA POSIZIONE INTERPOLATA PER EVITARE MOVIMENTI INDESIDERATI DOPO L'INTERRUZIONE
      spiStruct->pitchTX = last_interp_pitch; //  RIPARTO DALL'ULTIMA POSIZIONE INTERPOLATA PER EVITARE MOVIMENTI INDESIDERATI DOPO L'INTERRUZIONE
      prepareSPIData(spiStruct->rollTX, spiStruct->pitchTX, tx);
      send_to_manipulator(&spiManager);

      *ptr_flag = 2;  // indica che i dati dell'ultimo punto sono stati inviati al manipolatore 
      
      cleanup_trajectory();

      // ripristino le condizioni operative di default
      *ptr_flag = 6;      
      //current_state = FREE_HAND; questo switch è impostato al rilascio del pulsante sinistro
  ...
  if (current_state != ZERO_POINT && flag != 2 && current_state != SEND_LAST_POINT){
    // Resetto i buffer per la SPI ( memset(tx, 0, sizeof(tx)); )
    rx[0] = 0; tx[0] = 0;
    rx[1] = 0; tx[1] = 0;
    rx[2] = 0; tx[2] = 0;
    rx[3] = 0; tx[3] = 0;
    ...
    prepareSPIData(spiStruct->rollTX, spiStruct->pitchTX, tx);
    send_to_manipulator(&spiManager); // il pacchetto viene inviato solo se NON siamo già in SEND_LAST_POINT
...
void mouseMoved() {
  ...
  int32_t xChange_micro = (usbManager.mouse.getXChange()) * 250; // restituisce la variazione dell'asse X del mouse rispetto alla sua posizione precedente
  int32_t yChange_micro = (usbManager.mouse.getYChange()) * 250; // restituisce la variazione dell'asse Y del mouse rispetto alla sua posizione precedente
...
void mouseDragged(){
  ...
  if(flag == 5 || flag == 6){
    // evitiamo di far accendere ledFault in modo irreversibile
    mouseMoved();
  }else{
...
void send_to_manipulator(SPIManager* spiManager){
  ...
  switch(current_state){
    ...
    case ZERO_POINT:
      SPI1.beginTransaction(spiManager->spiSettings);
      digitalWrite(spiManager->csPin, LOW);                                 // Seleziona lo slave
      penSpi.rx_test[0] = SPI1.transfer16(DATA_ZERO_POINT);
      digitalWrite(spiManager->csPin, HIGH);                                // Deseleziona lo slave
      SPI1.endTransaction();
