...
extern  int32_t* ptr_last_interp_roll;
extern int32_t* ptr_last_interp_pitch;
extern  int32_t* ptr_last_recorded_roll;
extern int32_t* ptr_last_recorded_pitch;
...
boolean record_trajectory(trajectory_calm* traj, int32_t x_to_be_recorded, int32_t y_to_be_recorded, int32_t min_distance)
{
  if (traj->current_index_record < TRAJECTORY_MAX_SIZE) { // controllo per evitare overflow
    ...
    // Salva l'ultimo punto registrato
    *ptr_last_recorded_roll = static_cast<double>(x_to_be_recorded);
    *ptr_last_recorded_pitch = static_cast<double>(y_to_be_recorded);
...
boolean read_and_interp_trajectory(trajectory_calm* traj, int32_t* x, int32_t* y, double query_points_dist)
{
  ...
  if (traj->current_size > 4) 
  {
    ...
    // Controllo immediato del pulsante sinistro
    if (usbManager.mouse.getButton(LEFT_BUTTON)) {
      current_state = SEND_LAST_POINT;  // Interruzione immediata!
      return false;                      // Esce immediatamente dalla funzione
    }
    ...
    *y = static_cast<int32_t>(interpolated_y);
    ...
    // Salva l'ultimo punto interpolato
    *ptr_last_interp_roll = static_cast<int32_t>(interpolated_x);
    *ptr_last_interp_pitch = static_cast<int32_t>(interpolated_y);
